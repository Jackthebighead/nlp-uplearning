...
- ### **Languages**
    ...
    - the goal of NLP: to be able to design algorithms to allow computers to "understand" natural language in order to perform some task. Like machine translation, question answering, semantic analysis and so on.
...
- ### **Word Vectors**
    ...
    - use vector distance to represent the similarity of the context.
      - distance measurement like Jaccard, cosine, etc.
...
- ### **Word2Vec**
    ...
    - idea: create a model that will be able to learn one iteration at a time and eventually be able to encode the probability of a word given its context
    - basic process
        - use a large corpus
        ...
        - loop the process.
    - 2 algorithms and 2 training tricks
        - SG and CBOW
        - hierarchical softmax and negative sampling (talk in the Material3) 
    - model variants (2 algorithms: Skip-grams and CBOW)
        - a little history
            - language models
               - ideas: a model that assigns probability to a sequence of tokens.
               - unigram model: $P(w_1,w_2,...,w_n)=\prod_{i=1}^nP(w_i)$, but this loss the information of context.
               - bigram model: $P(w_1,w_2,...,w_n)=\prod_{i=1}^nP(w_i|w_{i-1})$, this model concerns the conditional probability of pair neighbor but still kind of naive, the word may depend on the context of many words around it.
        - Skip-grams
    ...
    - '2'
        ...
        - SG
           ...
           <img src="pics_lecture1/Untitled 4.png" width = "500" height = "350" alt="d" align=center />
            ...
            - First, input the one-hot vector (initialization) to the network, then look up the W matrix for vector encoding (dimension reduction), then use W' to calculate the context vector, then do a softmax to form the probability distribution. Feed the output to the Loss function, calculate the gradient and do back propagation to learn the parameters.
            - learn the W and W'
        - CBOW
            - CBOW is kind of the reverse version of SG model.
            - first generate word vectors in the form of one-hot, then get embedded word vectors from the context and average them to get the latent vector of the word, then multiply withe the weight matrix to get the logit score and softmax it to get the probability. finally we bp and learn the parameters of W and W'.
...
- ### **Material 3: Optimizations in Word2Vec**
    ...
    - Negative Sampling
        - make the model update some weights at a time rather than the whole paras.
            - computing softmax of each word by going through the whole corpus is computationly costy.
        - in order to save time, compared to the Hierarchical Softmax method.
        - process
            - for every correct example (take SG model for example, center word w_i and the context(w_i)), we need to sample neg negative examples (which is different center word w_j and context(w_i)), thus we can construct a logistic regression of '0' and '1' and use Sigmoid function to classify.
